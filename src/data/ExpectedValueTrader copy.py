...
class ExpectedValueTrader():
    ...
    def find_best_ev_credit_spreads(self, symbol, max_wingspan, tp=1.0, alpha=0.25, minTypeCredit = 0.2):
        try:
            """
            ...
            df = self.options_data[
                ...
                (self.options_data["delta"].abs() <= 0.41) &
    ...
    def calcExpectedValue(contract_rows, tp=0.5, alpha=0.25, simple=False):
        ...
        total_EV = 0
        if ev_put is not None and ev_put > 0:
            total_EV += ev_put

        if ev_call is not None and ev_call > 0:
            total_EV += ev_call
        
        return round(total_EV,2) if total_EV > 0 else None